...
public class UserService {
	...
	public String signup(User user, ModelMap map) throws IOException, MessagingException {
		...
		if (user1 == null && user2 == null) {
			...
			userDao.save(user);
			map.put("pass", "Registration Successful. Please Login.");
			return "UserLogin";
		} else {
	...
	public String forgotPassword(String email, ModelMap map) {
		...
		if (user == null) {
			...
			int otp = new Random().nextInt(100000, 999999);
			user.setOtp(otp);
			System.out.println("User Reset OTP: " + otp);
			// emailLogic.sendOtp(user);
			userDao.save(user);
	...
	public String applyJob(int id, User user, ModelMap map, HttpSession session) {
		...
		if (job == null) {
			...
			if (true) { // Bypass prime requirement
				// Logic for Applying
				if (true) { // Bypass profile requirement
